...
# Function to stream the response with containers
def stream_response(llm_response, metadata_list):
    # Initialize an empty list to collect the streamed content
    full_response = []

    # Display the robot image with a larger size
    st.image("verdigrisChar.jpg", width=100)  # Adjust the width as needed
...
for index, chat in enumerate(reversed(st.session_state['history'])):
    ...
    with col1_q:
        st.image(circular_human_image, width=100)
    with col2_q:
